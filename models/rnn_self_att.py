...
class Encoder(nn.Module):
    ...
    def forward(self, src, src_len):
        ...
        embedded = self.ff_layer_norm(embedded)
...
class Attention(nn.Module):
    def __init__(self, enc_hid_dim, dec_hid_dim, device):
        ...
        self.fc_k = nn.Linear(enc_hid_dim*2, hid_dim)

        self.dropout = nn.Dropout(0.1)

        self.scale = torch.sqrt(torch.FloatTensor([self.head_dim])).to(device)
        self.v = nn.Linear(self.n_heads, 1, bias=False)
    ...
    def forward(self, hidden, encoder_outputs, mask):
        ...
        energy = energy.view(batch_size, -1, self.n_heads)
...
class Decoder(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs, mask):
        ...
        if self.attention:
            ...
            rnn_input = embedded    # [1, batch size, embed_dim]
        rnn_input = self.ln(rnn_input)
